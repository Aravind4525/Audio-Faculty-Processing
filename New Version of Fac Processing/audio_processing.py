...
from fetch_videos import get_course_vids_secs, get_sections_for_course, get_videos_for_section
import requests
import boto3
import os
import re
...
def fetch_and_unionize_keywords(video_ids):
    ...
    all_keywords = set()
    try:
        all_critical_keywords = fetch_all_keywords(video_ids)
        flattened_keywords = [kw for sublist in all_critical_keywords for kw in (sublist if isinstance(sublist, list) else [sublist])]
        all_keywords.update(flattened_keywords)
    except Exception as e:
        print(f"Keyword fetch error for {video_ids}: {e}")
...
def process_audio(course_id, course_section_id: list, input_type="google_drive", input_source=None, server_type="dev"):
    """
    Main function to process audio files based on course_id and course_section_id and server_type.
    ...
    section_ids, video_content_ids = get_sections_for_course(course_section_id, server_type)
    video_ids = get_videos_for_section(video_content_ids, server_type)
    ...
    try:
        if video_ids:
            hardcoded_keywords = fetch_keywords(video_ids)
    ...
    num_keywords = max(1, math.ceil(len(sorted_keywords) * 0.6))
...
if __name__ == '__main__':
    ...
    result = process_audio(course_id=212, course_section_id=[9958, 9998], input_type="s3", input_source=["https://chatlms.s3.ap-south-1.amazonaws.com/audio_file4_0.mp3", "https://chatlms.s3.ap-south-1.amazonaws.com/audio_file4_1.mp3", "https://chatlms.s3.ap-south-1.amazonaws.com/audio_file4_2.mp3", "https://chatlms.s3.ap-south-1.amazonaws.com/audio_file4_3.mp3"])
