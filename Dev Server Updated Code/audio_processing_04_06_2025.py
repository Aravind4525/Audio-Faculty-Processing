...
def fetch_and_unionize_keywords(video_ids):
    ...
    video_ids = [
        ...
        for vid in (sublist if isinstance(sublist, list) else [sublist])
...
def process_audio(course_id, input_type="google_drive", input_source=None, server_type="dev"):
    ...
    if processed_transcripts:
        ...
        try:
            if video_ids:
                for video_id in video_ids[0]:
                    ...
                    flat_keywords.extend(str(keyword) for sublist in hardcoded_keywords for keyword in sublist)
                    flat_keywords = list(dict.fromkeys(flat_keywords))[:20]
        ...
        semantic_result = semantic_smart_answer(
            ...
            question=(  # Semantic matching question logic remains unchanged
                ...
                "Important: Output the response as raw JSON text without any Markdown formatting or code block delimiters (do not use triple backticks or any other enclosing characters). The JSON should be directly parseable."
            ...
            if 'content' in response:
                if isinstance(response['content'], str):
                    try:
                        ...
                        content_str = re.sub(r'"(.*?)"', fix_json_strings, content_str, flags=re.DOTALL)
                        # response['content'] = json.loads(response['content'])
                        response['content'] = json.loads(content_str)
    ...
    if result_list:
        result_list[0].update({
            ...
            # "trainer_questions": len(result['trainer_questions']),
            # "student_questions": len(result['student_questions']),
